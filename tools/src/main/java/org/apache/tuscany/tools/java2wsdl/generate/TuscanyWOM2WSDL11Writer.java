...
/**
 * This is a temporary replica of the WOM2WSDL11Writer in Axis2. There is a very
 * minor modification that was required in the method
 * populateMessageSymbol(Qname). Since the method uses the private instance
 * variable called messageMap merely inheriting from WOM2WSDL11Writer and
 * overriding the populateMessageSymbol() was not feasible.
 * 
 */
public class TuscanyWOM2WSDL11Writer implements WOMWriter {
	 ...
	 * @param wsdlWrapper
	 * @param out
	 * @see WOMWriter#writeWOM(org.apache.axis2.wsdl.WSDLVersionWrapper,
	 *      java.io.OutputStream)
	 */
	public void writeWOM(WSDLVersionWrapper wsdlWrapper, OutputStream out)
			throws WriterException {
		if (wsdlWrapper == null) {
			throw new WriterException("wsdl Wrapper cannot be null");
		}
		// the wsdl definition is present. then we can ditectly utilize the wsdl
		// writer.
		if (wsdlWrapper.getDefinition() != null) {
			...
			// wsdl description is present but not the definition. So start
			// writing
	 ...
	 * @param out
	 * @throws WriterException
	 * @see WOMWriter#writeWOM(org.apache.wsdl.WSDLDescription,
	 *      java.io.OutputStream)
	 */
	public void writeWOM(WSDLDescription wsdlDescription, OutputStream out)
			throws WriterException, XMLStreamException {
		try {
			...
			handleExtensibiltyElements(wsdlDescription
	 ...
	 * @throws IOException
	 */
	private void writeService(WSDLDescription wsdlDescription)
		...
		if (serviceMap != null && !serviceMap.isEmpty()) {
			...
			while (serviceIterator.hasNext()) {
				...
				writer.writeStartElement(defaultWSDLPrefix, "service",
	 ...
	 * @param endpoint
	 */
	private void writePort(WSDLEndpoint endpoint) throws IOException,
			XMLStreamException {
		writer.writeStartElement(defaultWSDLPrefix, "port",
				WSDL1_1_NAMESPACE_URI);
		writer.writeAttribute("name", endpoint.getName().getLocalPart());
		QName name = endpoint.getBinding().getName();
		writer.writeAttribute("binding", name.getPrefix() + ":"
				+ name.getLocalPart());
	 ...
	 * @throws IOException
	 */
	protected void writeStartDescripton(WSDLDescription desc,
			XMLStreamWriter writer) throws IOException, XMLStreamException {
		// always prefix the elements with wsdl1.1 prefix
		writer.writeStartElement(defaultWSDLPrefix, DEFINITION_NAME,
				WSDL1_1_NAMESPACE_URI);
		// write the name
		...
		if (namespaceMap != null && !namespaceMap.isEmpty()) {
			...
			while (nameSpaces.hasNext()) {
				...
				// make sure not to write the namespace URI of the WSDL 1.1
				// namespace
				if (!WSDL1_1_NAMESPACE_URI.equals(nsURI)
						&& !targetNameSpace.equals(nsURI)) {
					writeNamespace(nsPrefix, nsURI);
				}
	...
	private void writeNamespace(String namespacePrefix, String namespaceURI)
			throws XMLStreamException {
		// for the time being lets keep this simple as we do not expect that
		// much of namespaces within the WSDL
	...
	/**
	 * Writes the messages. This is somewhat tricky when the message names and
	 * parts have to be guessed.
	 * 
	 * @param desc
	 */
	protected void writeMessages(WSDLDescription desc)
			throws XMLStreamException {
		...
		if (mBag == null || mBag.isEmpty()) {
			...
			if (!interfaceMap.isEmpty()) {
				...
				while (interfaceIterator.hasNext()) {
					...
					if (!opMap.isEmpty()) {
						...
						while (opIterator.hasNext()) {
							...
							QName inputReference = operation.getInputMessage() == null ? null
							...
							QName outputReference = operation
									.getOutputMessage() == null ? null
				...
				// Now we are done with populating the message symbols. write
				// them down
				Iterator messages = messageMap.values().iterator();
				while (messages.hasNext()) {
					WSDL11Message msg = (WSDL11Message) messages.next();
					writer.writeStartElement(defaultWSDLPrefix, "message",
							WSDL1_1_NAMESPACE_URI);
					...
					for (int i = 0; i < parts.length; i++) {
						part = parts[i];
						writer.writeStartElement(defaultWSDLPrefix, "part",
								WSDL1_1_NAMESPACE_URI);
						writer.writeAttribute("name", part.getName());
						String elementName = part.getElementName();
						if (elementName != null) {
							// todo ajith pls take a look at that I have done
							// the correct thing here
							if (part.getPrefix() != null
									&& !part.getPrefix().trim().equals("")) {
								elementName = part.getPrefix() + ":"
										+ elementName;
							}
							writer.writeAttribute("element", elementName);
						}
						// put the type also here. For the time being let this
						// be like it
						writer.writeEndElement();
	 ...
	 * Message name is the localpart of the QName suffixed by
	 * MESSAGE_NAME_SUFFIX Partname is just 'part1' (wouldn't matter!) Element
	 * reference is again the localpart of the QName but prefixed with the
	 * target namespaces prefix.
	 */
	...
	private void populateMessageSymbol(QName reference) {
		if (!messageMap.containsKey(reference)) {
			...
			// these references need to be prefixed according to the correct
			// target namespaces
			// of the schemas
			...
			// todo prefix needs to be here!!!!

			/** ****** TUSCANY MODIFICATIONS START */
			// statement commented to remove existing behaviour
			// part.setPrefix(reference.getPrefix());
			// Statment added to modify behaviour
			part
					.setPrefix(Java2WSDLDefaultGenerator.schemaNamespace2GlobalPrefix
							.get(reference.getNamespaceURI()));
			/** ****** TUSCANY MODIFICATIONS END */

			WSDL11Message message = new WSDL11Message();
			message.setMessageName(reference.getLocalPart()
					+ MESSAGE_NAME_SUFFIX);
	...
	protected void writeEndDescripton(Writer outWriter) throws IOException {
		TuscanyWriterUtil.writeEndElement(DEFINITION_NAME, defaultWSDLPrefix,
				outWriter);
	}
	 ...
	 * @throws IOException
	 */
	protected void writeImports(WSDLDescription desc) throws IOException,
			XMLStreamException {
		...
		for (int i = 0; i < importCount; i++) {
			...
			writer.writeStartElement(defaultWSDLPrefix, IMPORT_NAME,
					WSDL1_1_NAMESPACE_URI);
	 ...
	 * @throws IOException
	 */
	protected void writeTypes(WSDLDescription desc) throws IOException,
			XMLStreamException {
		// get the imports
		WSDLTypes types = desc.getTypes();
		if (types != null) {
			writer.writeStartElement(defaultWSDLPrefix, TYPES_NAME,
					WSDL1_1_NAMESPACE_URI);
	 ...
	 * @throws IOException
	 */
	protected void writePortTypes(WSDLDescription desc)
		...
		if (!interfaceMap.isEmpty()) {
			...
			while (interfaceIterator.hasNext()) {
				wsdlInterface = (WSDLInterface) interfaceIterator.next();
				writer.writeStartElement(defaultWSDLPrefix, PORTTYPE_NAME,
						WSDL1_1_NAMESPACE_URI);
				writer.writeAttribute("name",
						wsdlInterface.getName() == null ? "" : wsdlInterface
								.getName().getLocalPart());
				// write extensibility attributes
				handleExtensibilityAttributes(wsdlInterface
	 ...
	 * @throws IOException
	 */
	protected void writePorttypeOperations(WSDLInterface wsdlInterface)
			throws XMLStreamException, IOException {
		...
		if (!operationsMap.isEmpty()) {
			...
			while (opIterator.hasNext()) {
				operation = (WSDLOperation) opIterator.next();
				writer.writeStartElement(defaultWSDLPrefix, OPERATION_NAME,
						WSDL1_1_NAMESPACE_URI);
				writer.writeAttribute("name", operation.getName() == null ? ""
				...
				if (inputMessage != null) {
					message = (WSDL11Message) messageMap.get(inputMessage
							.getElementQName());
					...
					// write extensibility attributes
					handleExtensibilityAttributes(inputMessage
							.getExtensibilityAttributes());
					writer.writeEndElement();
				}
				...
				if (outputMessage != null) {
					message = (WSDL11Message) messageMap.get(outputMessage
							.getElementQName());
					...
					writer.writeAttribute("message", targetNamespacePrefix
							+ ":" + message.getMessageName());
					// write extensibility attributes
					handleExtensibilityAttributes(outputMessage
							.getExtensibilityAttributes());
					writer.writeEndElement();
				}
	 ...
	 * @throws IOException
	 */
	protected void writeBinding(WSDLDescription desc)
			throws XMLStreamException, IOException {
		...
		if (!bindingsMap.isEmpty()) {
			...
			while (iterator.hasNext()) {
				...
				writer.writeStartElement(defaultWSDLPrefix, BINDING_NAME,
						WSDL1_1_NAMESPACE_URI);
				writer.writeAttribute("name", binding.getName().getLocalPart());
				writer.writeAttribute("type", targetNamespacePrefix + ":"
						+ binding.getBoundInterface().getName().getLocalPart());
				...
				if (bindingOps != null && !bindingOps.isEmpty()) {
					Iterator bindingOpsIterator = bindingOps.values()
							.iterator();
					while (bindingOpsIterator.hasNext()) {
						writebindingOperation((WSDLBindingOperation) bindingOpsIterator
								.next());
					}
				}
				writer.writeEndElement();
	...
	protected void writebindingOperation(WSDLBindingOperation bindingOp)
			throws XMLStreamException, IOException {
		writer.writeStartElement(defaultWSDLPrefix, BINDING_OPERATION,
				WSDL1_1_NAMESPACE_URI);
		...
		if (input != null) {
			//
			writer.writeStartElement(defaultWSDLPrefix, BINDING_INPUT,
					WSDL1_1_NAMESPACE_URI);
		...
		if (output != null) {
			writer.writeStartElement(defaultWSDLPrefix, BINDING_OUTPUT,
					WSDL1_1_NAMESPACE_URI);
	...
	protected void handleExtensibilityAttributes(List extAttributeList)
		...
		for (int i = 0; i < extAttributeCount; i++) {
			writeExtensibilityAttribute((WSDLExtensibilityAttribute) extAttributeList
					.get(i));
		}
	}
	...
	protected void handleExtensibiltyElements(List extElementList)
		...
		for (int i = 0; i < extensibilityElementCount; i++) {
			writeExtensibilityElement((WSDLExtensibilityElement) extElementList
					.get(i));
		}
	...
	private void writeSchemas(Element element) throws XMLStreamException {
		...
		XMLStreamReader xmlReader = XMLInputFactory.newInstance()
				.createXMLStreamReader(
						new ByteArrayInputStream(schemaTypes.getBytes()));
	...
	protected void writeExtensibilityAttribute(
			WSDLExtensibilityAttribute extAttribute) throws XMLStreamException {
		QName qname = extAttribute.getKey();
		QName value = extAttribute.getValue();

		writer.writeAttribute(qname.getPrefix(), qname.getNamespaceURI(), qname
				.getLocalPart(), value.getLocalPart());
	}
	...
	protected void writeExtensibilityElement(WSDLExtensibilityElement extElement)
		...
		if (extElement instanceof Schema) {
			...
			writer.writeComment(" Unknown extensibility element"
					+ extElement.toString());
		}

	}

	private void writeSOAPAddressExtensibilityElement(SOAPAddress address)
			throws XMLStreamException {
		writer.writeStartElement(soapNsPrefix, "address",
				WSDL1_1_SOAP_NAMESPACE_URI);
		writer.writeAttribute("location", address.getLocationURI());
		writer.writeEndElement();
	}

	protected void writeSOAPBindingExtensibilityElement(SOAPBinding soapBinding)
			throws XMLStreamException {
		writer.writeStartElement(soapNsPrefix, "binding",
				WSDL1_1_SOAP_NAMESPACE_URI);
	...
	protected void writeSOAPBodyExtensibilityElement(SOAPBody soapBody)
			throws IOException, XMLStreamException {
		writer.writeStartElement(soapNsPrefix, "body",
				WSDL1_1_SOAP_NAMESPACE_URI);
	...
	protected void writeSOAPHeaderExtensibilityElement(SOAPHeader soapHeader)
			throws XMLStreamException {
		writer.writeStartElement(soapNsPrefix, "header",
				WSDL1_1_SOAP_NAMESPACE_URI);
	...
	protected void writePolicyExtensibilityElement(
			PolicyExtensibilityElement policyExtensibilityElement)
			throws XMLStreamException {
		StAXPolicyWriter policyWriter = (StAXPolicyWriter) PolicyFactory
		...
		if (policyElement instanceof Policy) {
			...
			policyWriter.writePolicyReference((PolicyReference) policyElement,
					writer);
		}
	}

	/**
	 * Since we have no proper way to represent a WSDL 1.1 message, here's a
	 * simple bean class to represent it, at least for serializing.
	 */
