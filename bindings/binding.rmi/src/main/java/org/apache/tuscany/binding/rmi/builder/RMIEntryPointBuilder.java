...
import org.apache.tuscany.binding.rmi.config.RMIEntryPointContextFactory;
 ...
 */
@Scope("MODULE")
public class RMIEntryPointBuilder extends EntryPointBuilderSupport<RMIBinding> {

    
    protected EntryPointContextFactory createEntryPointContextFactory(EntryPoint entryPoint, MessageFactory msgFactory) 
    {
        RMIBinding rmiBinding = (RMIBinding) entryPoint.getBindings().get(0);
        return new RMIEntryPointContextFactory(entryPoint.getName(), messageFactory, rmiBinding);
    }
