/*
 * Tuscany JSON-RPC 
 * This is the jsonrpc.js file from JSON-RPC-Java 1.0rc2 renamed and 
 * with a single change which is right at the bottom of the script. 
...
function JSONRpcClient_default_ex_handler(e) {
	alert(e);
};
...
function JSONRpcClient_createMethod(methodName)
{
	var fn=function()
	{
		...
		var req = fn.client._makeRequest.call
			(fn.client, fn.methodName, args, callback);
		if(callback == null) {
...
function JSONRpcClient_handleResponse(http)
{
	...
	if(obj.error)
		throw new JSONRpcClient.Exception
			(obj.error.code, obj.error.msg, obj.error.trace);
	var res = obj.result;

	/* Handle CallableProxy */
	if(res && res.objectID && res.JSONRPCType == "CallableReference")
		return new JSONRpcClient
			(this.serverURL, this.user, this.pass, res.objectID);
...
function JSONRpcClient_getHTTPRequest()
{
	...
	throw new JSONRpcClient.Exception
		(0, "Can't create XMLHttpRequest object");
};

/* Tuscany: hack start */
   SCA = new JSONRpcClient("SCA/jsonrpc"); 
/* Tuscany: hack end */
